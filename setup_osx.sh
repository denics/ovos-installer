...
brew_update() { sudo -u $RUN_AS brew update &>/dev/null; }
ensure_brew() {
...
sudo -u $RUN_AS brew install coreutils dialog jq yq ansible fann

# shellcheck source=utils/constants.sh
source utils/constants.sh
export LOG_PATH=~/Library/Logs/ovos
export LOG_FILE=$LOG_PATH/ovos-installer.log
export RASPBERRYPI_MODEL="N/A"
export ANSIBLE_LOG_FILE=~/Library/Logs/ovos/ovos-ansible.log
export YQ_BINARY_PATH=/opt/homebrew/bin/yq
...
# Build the ansible-playbook command so it can be displayed
ANSIBLE_CMD="ansible-playbook -i 127.0.0.1, ansible/site.yml \
  ...
  -b \
  -e \"ansible_become_pass='%s'\" --ask-become-pass"

 echo "➤ Running Ansible: $ANSIBLE_CMD"
   $ANSIBLE_CMD
